...
export const ProjectFormModal: React.FC<ProjectFormModalProps> = ({ 
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
      <div className="w-full max-w-sm rounded-xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-200">
        <div className="flex items-center justify-between border-b border-zinc-200 dark:border-zinc-800 p-4">
          ...
          <button onClick={onClose} className="rounded-md p-1 text-zinc-500 hover:bg-zinc-100 dark:hover:bg-zinc-800 hover:text-zinc-900 dark:hover:text-white">
            <X size={16} />
          </button>
        </div>
        <div className="p-4 space-y-4">
          <div className="space-y-1.5">
            <label className="text-[11px] font-medium text-zinc-500 dark:text-zinc-400">{t('modal_name', lang)}</label>
            <input 
              type="text" 
              value={formData.name}
              onChange={e => setFormData(prev => ({...prev, name: e.target.value}))}
              placeholder="e.g. New Content Pack"
              className="w-full rounded-md border border-zinc-300 dark:border-zinc-700 bg-white dark:bg-zinc-950 px-3 py-2 text-xs text-zinc-900 dark:text-white placeholder-zinc-400 dark:placeholder-zinc-600 focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-indigo-500"
              autoFocus
            />
          </div>
          <div className="space-y-1.5">
            <label className="text-[11px] font-medium text-zinc-500 dark:text-zinc-400">{t('modal_desc', lang)}</label>
            <input 
              type="text" 
          ...
          <div className="space-y-1.5">
              <div className="flex justify-between items-center">
                <label className="text-[11px] font-medium text-zinc-500 dark:text-zinc-400">{t('modal_tags', lang)}</label>
              </div>
              <div className="relative">
                <Tag size={12} className="absolute left-3 top-2.5 text-zinc-500 dark:text-zinc-600" />
                <input 
                  type="text" 
                  value={formData.tagsString}
                  onChange={e => setFormData(prev => ({...prev, tagsString: e.target.value}))}
                  placeholder="e.g. NSFW, Bonus, Draft"
                  className="w-full rounded-md border border-zinc-300 dark:border-zinc-700 bg-white dark:bg-zinc-950 pl-8 pr-3 py-2 text-xs text-zinc-900 dark:text-white placeholder-zinc-400 dark:placeholder-zinc-600 focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-indigo-500"
                />
              </div>
          </div>
              ...
              {(Object.entries(categoryConfig) as [string, CategoryDefinition][]).map(([key, config]) => (
                <button
                  ...
                  className={`
                    ...
                    ${formData.type === key 
                      ? 'bg-zinc-100 dark:bg-zinc-800 border-indigo-500 text-zinc-900 dark:text-white shadow-md' 
        ...
        <div className="flex items-center justify-between border-t border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-900/50 p-4">
            {editingProject && onDelete ? (
              isDeleteConfirming ? (
                  <div className="flex items-center gap-2 mr-auto bg-red-50 dark:bg-red-950/30 px-2 py-1 rounded border border-red-200 dark:border-red-900/50">
                      <AlertTriangle size={12} className="text-red-500 dark:text-red-400" />
                      <span className="text-[10px] text-red-600 dark:text-red-200 font-medium">{t('modal_reallyDelete', lang)}</span>
                      <div className="flex gap-1 ml-1">
                          ...
                          <button 
                            ...
                            className="text-zinc-600 dark:text-zinc-300 bg-white dark:bg-zinc-800 hover:bg-zinc-100 dark:hover:bg-zinc-700 px-2 py-0.5 rounded text-[10px]"
                          >
                              NO
                          </button>
                      </div>
                  ...
                  <button 
                    ...
                    className="text-red-500 hover:text-red-600 dark:text-red-400 dark:hover:text-red-300 hover:bg-red-50 dark:hover:bg-red-950/50 p-1.5 rounded transition-colors group"
          ...
          <div className="flex items-center gap-2">
            <button onClick={onClose} className="rounded-md px-3 py-1.5 text-xs font-medium text-zinc-500 hover:bg-zinc-100 dark:hover:bg-zinc-800 hover:text-zinc-900 dark:hover:text-white">{t('modal_cancel', lang)}</button>
            <button 
